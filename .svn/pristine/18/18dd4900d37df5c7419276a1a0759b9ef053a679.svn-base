<template>
  <div class="information">
    <ul>
      <li  v-for="(item1,index) in zxList" :key="index">
        <h3 @click="zxClick(item1.ID)">
          <img :src="checkImg(item1.app_logo)" alt="">
        </h3>
        <p class="li-title" @click="zxClick(item1.ID)">{{item1.post_title}}</p>
    ...
    methods:{
        getDate:function () {
            if(this.$store.state.online.onlineAppHomeList == null){
                ...
                this.zxList = JSON.parse(this.$store.state.online.onlineAppHomeList.zx)
            ...
            _that.$axios.post(serviceUrl + 'zxyy')
                .then(function(res){
                    if(res.data.code == 200){
                        //最新查询
                        _that.zxList = JSON.parse(res.data.data.zx)
