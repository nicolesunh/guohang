<template>
  <div class="latestApplication">
    <ul>
      <li v-for="(item1,index) in zxyyList" :key="index">
          <h3 @click="zxyyClick(item1.ID)">
              <img :src="checkImg(item1.app_logo)" alt="">
          </h3>
          <p class="li-title" @click="zxyyClick(item1.ID)">{{item1.post_title}}</p>
    ...
    methods:{
        getDate:function () {
            if(this.$store.state.online.onlineAppHomeList == null){
                ...
                this.zxyyList = JSON.parse(this.$store.state.online.onlineAppHomeList.zxyy)
            }
        },
        getDate2:function () {
            var _that  = this
            _that.$axios.post(serviceUrl + 'zxyy')
                .then(function(res){
                    if(res.data.code == 200){
                        //最新查询
                        _that.zxyyList = JSON.parse(res.data.data.zxyy)
