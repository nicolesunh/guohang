<template>
  <div class="ShippingDevSupportList">
    <div class="ShippingDevSupportList-list">
      <ul class="">
        <li  v-for="(item1,index) in dateList" :key="index">
          <h3 @click="supportClick(item1.ID)">{{item1.post_title}}</h3>
          <p class="li-bd">
              {{item1.post_content}}
            <span  @click="supportClick(item1.ID)"><img src="../../.././assets/img/readmore.png" alt=""></span>
          </p>
          <p>
    ...
    <!--分页-->
    <div class="ShippingDevSupportList_page">
      <div class="block">
        <el-pagination
          background
  ...
  export default {
    ...
    methods:{
        ...
        getPageDate:function () {
            var _that  = this
            _that.$axios.post(serviceUrl + 'hyfz_zczc',
                _that.qs.stringify({
...
<style lang = scss scoped>
  .ShippingDevSupportList{
    width: 100%;
    .ShippingDevSupportList-list{
      min-height: 500px;
      ul{
        padding-right: 15px;
        li{
